...
const CyberCoreLoader = () => {
  ...
  useEffect(() => {
    ...
    const ROTATION_SPEED = 0.018; // Smooth, continuous spin speed (rad/frame)
    ...
    for (let i = 0; i < coreCount; i++) {
      ...
      const targetR = Math.pow(Math.random(), 0.6) * 75;
    ...
    // 2. DENSE GEODESIC SHELL (Radius 250px, ~1800 Nodes)
    const radius = 250;
    const lats = 36;
    const lons = 50;
    ...
    for (let i = 0; i < spikeCount; i++) {
      ...
      const tX1 = radius * Math.sin(phi) * Math.cos(theta);
      const tY1 = radius * Math.sin(phi) * Math.sin(theta);
      const tZ1 = radius * Math.cos(phi);

      const tX2 = (radius + len) * Math.sin(phi) * Math.cos(theta);
      const tY2 = (radius + len) * Math.sin(phi) * Math.sin(theta);
      const tZ2 = (radius + len) * Math.cos(phi);
    ...
    const animate = (now) => {
      ...
      // Continuous rotation frame-by-frame (never stops)
      rotY += ROTATION_SPEED;
      const rotX = 0.25;
      ...
      for (let i = 0; i < projectedShell.length; i++) {
        ...
        const nodeAlpha = (p1.z + radius) / (radius * 2);
      ...
      outerSpikes.forEach((s) => {
        ...
        const spikeAlpha = (p1.z + radius) / (radius * 2);
...
const styles = {
  ...
  canvas: {
    ...
    filter: 'drop-shadow(0 0 22px rgba(0, 243, 255, 0.95))',
  },
  progressContainer: {
    position: 'absolute',
    bottom: '40px',
